 ...
 *
 * 字段分两类（D1）：
 * - `goal`/`mustDo`/`mustAvoid` 是**行为规则的权威文本**，
...
type StrategyRuleText = Pick<StrategyPolicy, "goal" | "mustDo" | "mustAvoid">;

/** Lab 内可编辑的数值部分。 */
type StrategyTunables = Pick<
  StrategyPolicy,
  "defaultMaxActions" | "lengthMultiplier" | "allowWorldview"
>;
...
> = {
  COMPANION: {
    ...
    mustAvoid: [
      "原因分析",
      "无请求建议",
      "无情绪载荷闲聊时用许可式陪伴套话（闲也挺好／不用硬找话题／我就待着）",
      "用户把话题权交给你时把问题再丢回用户",
    ],
  },
  ASK_LIGHT: {
